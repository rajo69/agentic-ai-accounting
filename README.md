...
|---|---|
| **Transaction categorisation** | LangGraph agent classifies Xero transactions against the chart of accounts using Claude + pgvector few-shot examples drawn from the firm's own history. High-confidence results are applied automatically; uncertain results are surfaced for human review. |
...
| **Management letter generation** | RAG pipeline computes financial figures in pure Python (no LLM), retrieves relevant transaction context via pgvector, then uses Claude + Instructor to write structured narrative sections. WeasyPrint renders the result as a professional A4 PDF. |
...
┌──────────────────────────▼───────────────────────────────────┐
│                  Next.js 14 (Vercel)                          │
│   Landing · Dashboard · Transactions · Reconciliation · Docs  │
└──────────────────────────┬───────────────────────────────────┘
                           │ REST / JSON
┌──────────────────────────▼───────────────────────────────────┐
│                    FastAPI (Railway)                           │
...
│  │/dashboard│  │/status │  │/transactions│  │/bank-stmts  │  │
│  └──────────┘  └────────┘  └────────────┘  └─────────────┘  │
│                                                               │
│  ┌───────────────────────────────────────────────────────┐   │
│  │                   Service Layer                        │   │
...
│  ┌───────────────────────────────────────────────────────┐   │
│  │                   XAI Engine                           │   │
│  │      EBM Explainer · Simpful Fuzzy Risk Scorer        │   │
│  └───────────────────────────────────────────────────────┘   │
└──────┬──────────────────────────┬────────────────────────────┘
       │                          │
┌──────▼──────────┐    ┌──────────▼──────────┐
...
Xero API ──sync──► transactions table (uncategorised)
       ...
       (5 nearest     (classify      (every decision,
       categorised    + reason)       full XAI payload)
       examples)
           │
     confidence > 0.85 ──► auto_categorised (written to DB)
     confidence 0.5–0.85 ─► suggested (shown with Accept/Reject)
     confidence < 0.5 ────► needs_review (flagged, no guess shown)
```
...
- **OAuth2 flow**: Redirects the user to Xero, receives the auth code, exchanges it for access + refresh tokens, and stores them against the `Organisation` record.
...
- **Rate-limit backoff**: Xero allows 60 requests/minute. `_get_with_retry()` catches HTTP 429 and waits 60 seconds before retrying, up to 3 attempts.
...
- The full chart of accounts (code + name)
...
2. Is `confidence` in `[0.0, 1.0]`?

If either fails, confidence is set to 0 and the reasoning is overwritten with "validation failed". The decision node will then flag this transaction for human review rather than making a bad auto-categorisation.
...
|---|---|---|
| > 0.85 | `auto_categorised` | Written to DB immediately, no human needed |
| 0.50 – 0.85 | `suggested` | Shown to accountant with Accept/Reject buttons |
| < 0.50 | `needs_review` | Flagged; no suggestion shown (a confusing low-confidence guess is worse than no guess) |
...
When an accountant corrects a categorisation via `POST /transactions/{id}/correct`, the transaction is re-embedded and the new embedding is stored. The next time a similar transaction arrives, the corrected version appears in `similar_examples` — the vector store acts as the training set. No fine-tuning, no retraining, no pipeline.

#### Batch runner

`categorise_batch()` fetches all uncategorised transactions and processes them concurrently with `asyncio.Semaphore(5)`, preventing Xero API rate-limit issues and keeping Claude API costs predictable.
...
```
find_candidates ──► score_candidates ──► decide ──► explain ──► save
```
...
| > 0.9 | `auto_matched` |
| 0.6 – 0.9 | `suggested` |
| < 0.6 or two candidates within 0.05 of each other | `needs_review` |
...
Claude's `reasoning` field from the `CategoryPrediction` output. Plain English: *"This is likely a software subscription based on the ADOBE description, the £47.99 recurring amount, and the fact that 3 similar transactions have been categorised as Computer Equipment & Software."*

#### Layer 2 — InterpretML EBM feature importances

Once ≥50 labelled transactions exist in the organisation, an Explainable Boosting Machine is trained on five features:
...
A Mamdani-style fuzzy inference system implemented without Simpful as an external dependency — the membership functions and inference engine are implemented directly in `fuzzy_engine.py` using triangular membership functions and centroid defuzzification.

**Three input variables** (all normalised to [0, 1]):

| Variable | What it measures | Low → High |
|---|---|---|
| `amount_deviation` | How far the amount deviates from the category average (z-score normalised) | Expected → Anomalous |
| `vendor_frequency` | How often this vendor appears in transaction history | Rare → Frequent |
| `time_pattern` | Whether the transaction date is a weekday (0.9) or weekend (0.3) | Unusual → Normal |
...
**Why fuzzy logic over a neural anomaly detector**: Fuzzy rules are human-readable, deterministic (same input → same output always), and transparent — an accountant can review and challenge the rules. A neural anomaly detector would be a black box. For an accounting tool where professional trust is the product, interpretability beats marginal accuracy gains.

**Output**: `risk_score ∈ [0, 1]`, `risk_label ∈ {low, medium, high}`, and the list of fired rules in plain English — shown directly in the UI so the accountant understands exactly why a transaction was flagged.
...
The LLM never touches the numbers directly — it writes around figures that have already been computed and verified.

**Step 4 — Render PDF (Jinja2 + WeasyPrint)**
A Jinja2 HTML template is populated with figures and narrative, then rendered to A4 PDF with WeasyPrint. Includes page numbers, professional typography, and an "AI-Assisted Draft" watermark footer. WeasyPrint was chosen over Puppeteer (too heavy for Python backend) and ReportLab (requires programmatic x/y layout — HTML/CSS is far more natural for structured documents).
...
| String matching | RapidFuzz | C-extension reimplementation of fuzzywuzzy; 10–100x faster, no GPL licence issues |
...
**Financial amounts are always `NUMERIC(12,2)` / Python `Decimal`.** IEEE 754 float cannot represent 0.1 exactly; a rounding error that causes a penny discrepancy in a reconciliation match is a critical bug in an accounting tool. Float is never used for money.
...
`fixtures/transactions.json` — 50 labelled UK SME bank transactions:
...
The agent must pass these before any deployment:
...
- Default eval model is `claude-haiku-4-5-20251001` (~10x cheaper than Sonnet). Switch to Sonnet only for pre-deployment quality checks
...
The test suite mocks external API calls (Xero, Anthropic) so tests run offline without credentials.
...
| Customers | Transactions/mo | API cost/mo |
|---|---|---|
| 5 (beta) | 1,000 | ~$0.52 |
| 20 | 4,000 | ~$2.06 |
| 100 | 20,000 | ~$10.30 |

At 100 customers paying £49/month, API cost is under 0.2% of revenue.

### Cost optimisation mechanisms

1. **Auto-categorisation reduces volume** — as the pgvector few-shot store grows, more transactions hit the >0.85 threshold and are categorised without prompting. Cost per new transaction trends toward zero as patterns repeat.
...
**Categorisation accuracy depends on history volume.** The few-shot pgvector lookup improves as the accountant corrects predictions and the vector store grows. A brand-new organisation with zero labelled transactions gets only chart-of-accounts context — accuracy will be lower until ~50 confirmed transactions exist. The eval framework's acceptance criteria account for this, but new users should expect a short calibration period.

**Confidence thresholds are not calibrated per client.** The 0.85 / 0.50 thresholds were chosen based on the evaluation fixture set. Different firms have different transaction patterns, and a threshold that works well for a retail client may be too aggressive for a construction firm with irregular payments. Threshold tuning per organisation is planned but not implemented.

**Fuzzy risk scoring uses heuristic input normalisation.** `vendor_frequency` is normalised by dividing raw count by 20 (i.e., 20+ appearances = "frequent"). This cap is arbitrary and may not fit all organisation sizes. A firm with 5,000 transactions/month will have different frequency distributions than one with 200.

**Xero sync is full-page, not incremental.** `sync_transactions()` fetches all pages on every sync. This is acceptable for SMEs but will be slow (several minutes) for organisations with thousands of transactions. `If-Modified-Since` incremental sync is designed but not yet implemented.
...
**Single tenant per Xero organisation.** One Xero firm = one tenant. Multi-user access within a firm (e.g., multiple staff at one accounting practice sharing the tool) is not implemented. All queries are scoped to `organisation_id` only — user-level permissions within an organisation are post-beta.

**EBM requires 50+ labelled samples.** Below this threshold, Layer 2 (feature importance) falls back to the LLM's reasoning text. New organisations will rely solely on Layers 1 and 3 (LLM reasoning + fuzzy risk) until sufficient data accumulates.

**PDF rendering is CPU-intensive.** WeasyPrint renders HTML to PDF synchronously. For large management letters it can take 2–5 seconds. This is acceptable for an on-demand operation but would block the event loop if called frequently. It should be moved to a background worker (Celery or Railway background worker) before scaling.
...
| Every AI decision → `AuditLog` | Professional accountability; GDPR right to explanation |
